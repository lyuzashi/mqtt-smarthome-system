// Modified for use with JSON2D
// https://github.com/dobarkod/canvas-bezier-multiple
export default (points, tension) => {
  const instructions = [];
  ...
  if (l < 2) return instructions;

  // If we only have two points, we can only draw a straight line
  if (l == 2) {
    instructions.push(['moveTo', points[0][0], points[0][1]]);
    instructions.push(['lineTo', points[1][0], points[1][1]]);
    return instructions;
  }
  ...
  for (i = 1; i < l; i++) {
    ...
    instructions.push(['bezierCurveTo', cpp.cn[0], cpp.cn[1], cp.cp[0], cp.cp[1], p[0], p[1]]);
  }
